...
const BannerPicker = ({ response, message, applicationNumber, info }) => {
  if (response) {
    return (
      <Banner
        message={message}
...
const Response = () => {
  ...
  const responseData = useSelector((state) => state?.fa?.common?.responseData || state?.common?.responseData);
    ...
    <Card style={{ margin: "10px" }}>
      {responseData ? (
        <BannerPicker
          ...
          applicationNumber={responseData.createdId}
      ...
      >
        ...
        {responseData?.secondaryRedirectionLabel && responseData?.onSecondaryRedirection && (
          <SubmitBar
            ...
            headerStyle={{ fontSize: "20px", fontWeight: "500" }}
