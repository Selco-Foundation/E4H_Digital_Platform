...
const FacilityActionBar = ({ t, selectedFacilityIds, bulkActions, onDownload }) => {
  ...
  const selectedCount = selectedFacilityIds?.length || 0;
    ...
    <div style={{ marginBottom: "10px" }}>
      ...
      <div
        style={{
          ...
          color: "#6B7280",
          marginTop: "8px",
          visibility: (selectedCount > 0 && bulkActions?.length > 0) ? "visible" : "hidden",
        }}
      >
        {selectedCount} {t("PM_ASSESSMENT_FACILITIES_SELECTED")}
      </div>
    </div>
  );
};
