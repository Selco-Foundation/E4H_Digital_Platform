...
const useFacilityAsset = (assetQueryFilter, limit = 1000, offset = 0) => {
  ...
  const { isLoading, isError, error, data } = useQuery(
    ["FACILITY_ASSET", filter, limit, offset],
    () => fetchAssets(filter, limit, offset)
  );

  return {
    isLoading, isError, error, data,
    revalidate: () => queryClient.invalidateQueries(["FACILITY_ASSET"])
  }
}

export default useFacilityAsset;
