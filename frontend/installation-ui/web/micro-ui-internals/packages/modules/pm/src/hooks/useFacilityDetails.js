...
const fetchFacilityDetails = async (queryFilter) => {
  ...
  return {
    ...
    facilityPocUsername: facility?.facility_poc_username,
...
const useFacilityDetails = (id) => {
  ...
  const { isLoading, isError, error, data } = useQuery(
    ["FACILITY_REGISTRY_DETAILS", queryFilter],
  ...
  return {
    ...
    error,
    data,
    revalidate: () => queryClient.invalidateQueries(["FACILITY_REGISTRY_DETAILS"]),
  };
};
