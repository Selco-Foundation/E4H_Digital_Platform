...
const fetchAMCVisits = async (filter, limit, offset) => {
  const amcVisitsResponse = await AMCVisitService.fetchVisits(filter, limit, offset);
  return {
...
const useFacilityAMCVisit = (amcConfigurationId) => {
  ...
  const { isLoading, isFetching, isError, error, data } = useQuery(
    ["FACILITY_AMC_VISIT", filter, limit, offset],
    () => fetchAMCVisits(filter, limit, offset)
  );

  return {
    isLoading, isFetching, isError, error, data,
    revalidate: () => queryClient.invalidateQueries(["FACILITY_AMC_VISIT"]),
  };
};

export default useFacilityAMCVisit;
