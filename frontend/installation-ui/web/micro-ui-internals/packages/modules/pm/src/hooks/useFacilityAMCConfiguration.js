...
const fetchAMCAssetSerialNumbers = async (amcConfigurationId) => {
  ...
  const amcAssetsResponse = await AMCConfigurationService.fetchAMCAssets(queryFilter, 100, 0);
...
const fetchAMCConfigurations = async (filter, limit, offset) => {
  const amcConfigurationsResponse = await AMCConfigurationService.fetchAMCConfigurations(filter, limit, offset);
  return {
...
const useFacilityAMCConfiguration = (amcConfigurationQueryFilter, pageSize, pageOffset) => {
  ...
  const { isLoading, isFetching, isError, error, data } = useQuery(
    ["FACILITY_AMC", filter, limit, offset],
    () => fetchAMCConfigurations(filter, limit, offset)
  );

  return {
    isLoading, isFetching, isError, error, data,
    revalidate: () => queryClient.invalidateQueries(["FACILITY_AMC"]),
  };
};

export default useFacilityAMCConfiguration;
