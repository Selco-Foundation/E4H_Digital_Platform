...
export const getInstallationReportFilename = (facilityName, projectName) => {
  const sanitize = (value) => String(value).replace(/[<>:"/\\|?*\x00-\x1f]/g, "_").trim();
  return `${sanitize(facilityName)}_${sanitize(projectName)}.pdf`;
};
...
export const createInstallationReportZip = (documents) => {
  ...
  for (const { report, data } of documents) {
    const filename = getInstallationReportFilename(report.facilityName, report.projectName);
