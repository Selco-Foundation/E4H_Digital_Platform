...
const useFeedBackSearch = ({ tenantId, filters }, config = {}) => {
  ...
  let serviceSearchArg = {
    filters: { ServiceCriteria: { tenantId: filters?.serviceSearchArgs?.tenantId, referenceIds: filters?.serviceSearchArgs?.referenceIds } },
  };
  ...
  const { isLoading: serviceLoading, error: serviceerror, data: servicedata } = useQuery(
    ["ServiceSearch", tenantId, filters],
  ...
  return {
    ...
    error: serviceerror,
    data: servicedata,
    revalidate: () => client.invalidateQueries(["ServiceSearch", tenantId, filters]),
  };
};
