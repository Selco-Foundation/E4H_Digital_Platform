...
const useReceiptsMDMS = (tenantId, type, config = {}) => {
  const useReceiptsBusinessServices = () => {
    ...
    if (!isLoading && data && data[`common-masters`] && data[`common-masters`]?.uiCommonPay && Array.isArray(data[`common-masters`].uiCommonPay)) {
      ...
      data.dropdownData =
        [
          ...data[`common-masters`].uiCommonPay.map((config) => {
            return {
              code: config.code,
  ...
  const useCancelReceiptReason = () => {
    const { isLoading, error, data } = useQuery(
      ["RECEIPTS_CANCEL_REASON", tenantId],
      () => MdmsService.getCancelReceiptReason(tenantId, "common-masters"),
      config
    );
    if (
      !isLoading &&
      data &&
      data[`common-masters`] &&
      data[`common-masters`]?.CancelReceiptReason &&
      Array.isArray(data[`common-masters`].CancelReceiptReason)
    ) {
      data[`common-masters`].CancelReceiptReason = data[`common-masters`].CancelReceiptReason.filter((unit) => unit.active) || [];
      data.dropdownData =
        [
          ...data[`common-masters`].CancelReceiptReason.map((config) => {
            return {
              code: config.code,
  ...
  const useCancelReceiptStatus = () => {
    const { isLoading, error, data } = useQuery(
      ["RECEIPTS_CANCEL_STATUS", tenantId],
      () => MdmsService.getReceiptStatus(tenantId, "common-masters"),
      config
    );
    if (
      !isLoading &&
      data &&
      data[`common-masters`] &&
      data[`common-masters`]?.ReceiptStatus &&
      Array.isArray(data[`common-masters`].ReceiptStatus)
    ) {
      data[`common-masters`].ReceiptStatus = data[`common-masters`].ReceiptStatus.filter((unit) => unit.active) || [];
      data.dropdownData =
        [
          ...data[`common-masters`].ReceiptStatus.map((config) => {
            return {
              code: config.code,
  ...
  const useCancelReceiptReasonAndStatus = () => {
    const { isLoading, error, data } = useQuery(
      ["RECEIPTS_CANCEL_REASON_STATUS", tenantId],
      () => MdmsService.getCancelReceiptReasonAndStatus(tenantId, "common-masters"),
      config
    );
    if (!isLoading && data && data[`common-masters`] && data[`common-masters`]?.uiCommonPay && Array.isArray(data[`common-masters`].uiCommonPay)) {
      data[`common-masters`].uiCommonPay = data[`common-masters`].uiCommonPay.filter((unit) => unit.cancelReceipt) || [];
