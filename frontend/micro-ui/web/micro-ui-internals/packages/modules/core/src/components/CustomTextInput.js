...
import {MyLocation, VisibilityOff, Visibility, Search, CalendarToday} from "@egovernments/digit-ui-svg-components"
...
const CustomTextInput = (props) => {
  ...
  const inputClassNameForMandatory = `${
    user_type ? "digit-employee-card-inputError" : "digit-card-inputError"
  } ${props.disabled ? "disabled" : ""} ${props.customClass || ""} ${
    props.nonEditable ? "noneditable" : ""
  }  ${props.type === "numeric" ? "numeric" : ""}`;

  const inputClassName = `${
    user_type ? "digit-employee-card-input" : "digit-citizenCard-input"
  } ${props.disabled ? "disabled" : ""} focus-visible ${
    props.errorStyle ? "digit-employee-card-inputError" : ""
  } ${props.nonEditable ? "noneditable" : ""} ${
    props.type === "numeric" ? "numeric" : ""
  } ${props.customClass || ""}`;
  ...
  return (
    <React.Fragment>
      <div
        className={`digit-text-input-field ${user_type === "employee" ? "" : "digit-text-input-field-width"} ${
          props?.className ? props?.className : ""
        } ${props.disabled ? "disabled" : ""}  ${props.nonEditable ? "noneditable" : ""} ${props.error ? "error" : ""} ${
          defaultType ? defaultType : ""
        } ${props?.populators?.disableTextField ? "numeric-buttons-only" : ""} ${props?.populators?.prefix ? "prefix" : ""} ${
        ...
        {props.type === "date" && props?.populators?.newDateFormat ? (
          <div className={inputContainerClass}>
            ...
            <div style={{ position: "relative", width: "100%" }}>
              <DatePicker
                ...
                placeholderText={StringManipulator("TOSENTENCECASE", t(props.placeholder))}
                locale={Digit?.SessionStorage.get("locale")}
                dateFormat="dd MMMM yyyy"
                className={props.required ? inputClassNameForMandatory : inputClassName}
                ...
                minDate={props?.populators?.min ? new Date(props?.populators?.min) : undefined}
                maxDate={props?.populators?.max ? new Date(props?.populators?.max) : undefined}
              />
              <div className={`digit-new-date-format ${props.disabled ? "disabled" : ""}`} onClick={() => datePickerRef.current?.setOpen(true)}>
                <CalendarToday fill={"#505A5F"} />
              </div>
            </div>
          ...
          <div className={inputContainerClass}>
            ...
            <input
              ...
              placeholder={StringManipulator("TOSENTENCECASE", t(props.placeholder))}
              onChange={(event) => {
