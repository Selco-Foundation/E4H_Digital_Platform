...
const CancelBills = ({ tenantId, onSubmit, data, count,isLoading,resultOk }) => {
    ...
    const handleBillLinkClick = (row) => {
        history.push(`/${window.contextPath}/employee/bills/bill-details?connectionNumber=${row?.consumerCode}&tenantId=${tenantId}&service=${row?.businessService}&from=ABG_CANCEL_BILL`,row)
    }
    ...
    const handleCancelBillAction = (_data) => {
        ...
        history.push(`/${window.contextPath}/employee/bills/response-cancelBill`, {filters,currentBill});
