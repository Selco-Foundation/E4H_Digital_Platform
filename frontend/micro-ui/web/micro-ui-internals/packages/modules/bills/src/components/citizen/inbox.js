        ...
        return (
          <div>
            <span className="link">
              <Link
                to={{
                  pathname: `/${window.contextPath}/citizen/payment/my-bills/${row.original?.["businessService"]}/${row.original?.["consumerCode"]}/tenantId=${row.original?.["tenantId"]}?workflow=mcollect`,
                }}
              >
                {t(`${"ABG_PAY"}`)}
              </Link>
