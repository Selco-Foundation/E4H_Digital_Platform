            ...
            {Object.keys(row.mapping).map((key) => {
              ...
              if (row.additionalCustomization[key]) {
                toRender = (
                  <Details
                    label={t(key)}
                    name={Digit?.Customizations?.[apiDetails?.masterName]?.[apiDetails?.moduleName]?.additionalCustomizations(
                      row.details,
                      key,
                      {},
                      row.mapping[key],
                ...
                toRender = row.mapping[key] ? (
                  <Details label={t(key)} name={row.mapping[key]} onClick={() => {}} row={row.mapping} />
                ) : (
                  <Details label={t(key)} name={t("NA")} onClick={() => {}} row={row.mapping} />
                );
              }
              return toRender;
            })}
