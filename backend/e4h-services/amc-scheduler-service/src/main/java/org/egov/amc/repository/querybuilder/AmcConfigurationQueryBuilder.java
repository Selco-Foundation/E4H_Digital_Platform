...
public class AmcConfigurationQueryBuilder {
    ...
    private static final String AMC_CONFIGURATION_COUNT_QUERY = "SELECT COUNT(*) FROM amc_configuration AS ac LEFT JOIN facility AS f ON ac.facility_id = f.id LEFT JOIN project AS p ON ac.project_id = p.id LEFT JOIN amc_configuration_assignments aca ON ac.id = aca.amc_configuration_id ";
