...
- **Kafka** — consumes video-processing requests; no results/events are published back to Kafka (see Events below).
- **ffmpeg / ffprobe** — external CLI binaries invoked via `Runtime.exec`/`ProcessBuilder` (`FFMpegExecutor`, `VideoUtil`). These are not bundled with the service — they must be installed on the runtime host/image and reachable on `PATH` (`ffprobe` path is explicitly configurable; `ffmpeg` is invoked by bare command name).
...
- Non-video files (anything not ending in `.mp4`, `.avi`, `.mov`, `.wmv`) are skipped entirely — only images/other file types accompanying a request are ignored, not processed or re-emitted.
...
Requires Java 17, Maven, and `ffmpeg`/`ffprobe` installed and reachable on the host (or set `ffprobe.path` to the actual binary location).

```bash
cd backend/e4h-services/processor-services

# build (skip tests if there's no local Kafka/filestore for integration tests)
mvn clean install -DskipTests

# run (defaults to port 8099, context path /processor-services)
mvn spring-boot:run

# or run the packaged jar
java -jar target/processor-services-0.0.1-SNAPSHOT.jar
```

A local Kafka broker (`kafka.config.bootstrap_server_config`, default `localhost:9092`) and a reachable egov-filestore instance (`egov.filestore.host`, default `http://localhost:8083`) are required for the consumer to receive and process real video jobs. Override properties via `application.properties`, environment variables, or `--spring.config.location` as needed for your environment.
