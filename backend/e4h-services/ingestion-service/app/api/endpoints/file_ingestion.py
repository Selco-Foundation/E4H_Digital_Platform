...
from fastapi import APIRouter, File, Form, UploadFile, HTTPException, BackgroundTasks, Depends, Body
from openpyxl import load_workbook
        ...
        for index, row in df.iterrows():
            try:
                ...
                try:
                    ...
                    df.at[index, 'status'] = 'success'
                    df.at[index, 'error'] = ''
                    logger.info(f"Successfully created AMC configuration for facility {facility_id}, vendor {vendor_name}")
                except Exception as e:
                    df.at[index, 'status'] = 'failed'
                    df.at[index, 'error'] = str(e)
