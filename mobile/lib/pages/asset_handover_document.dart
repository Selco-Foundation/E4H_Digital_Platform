...
@RoutePage()
class AssetHandoverDocumentPage extends StatefulWidget {
  ...
  @override
  State<AssetHandoverDocumentPage> createState() =>
      _AssetHandoverDocumentPageState();
}

class _AssetHandoverDocumentPageState extends State<AssetHandoverDocumentPage> {
  static const int _maxFiles = 3;

  List<ExistingReport> _existingHandoverDocumentFiles = [];
  ...
  bool _isInitialHandoverDocumentLoading = false;
  ...
  int get _totalSelectedCount =>
      _existingHandoverDocumentFiles.length + _selectedCount;
  ...
  int get _remainingImageSlots {
    final remaining = _maxFiles -
        _existingHandoverDocumentFiles.length -
        _selectedPdfFiles.length;
  ...
  void _loadCachedFiles() {
    setState(() {
      _isInitialHandoverDocumentLoading = true;
    });
    context.read<CacheAssetHandoverDocumentBloc>().add(
          CacheAssetHandoverDocumentEvent.load(
  ...
  Future<void> _populateFromCache(
    List<CacheAssetHandoverDocument> entries,
  ) async {
    final sorted = List<CacheAssetHandoverDocument>.from(entries)
    ...
    for (final entry in sorted) {
      var fileType = normalizeHandoverDocumentFileType(entry.fileType);
      if (fileType == 'unknown') {
        fileType = normalizeHandoverDocumentFileType(
    ...
    if (!mounted) return;
    setState(() {
      _existingHandoverDocumentFiles = existingFiles;
      _selectedImages = [];
      _selectedPdfFiles = [];
      _cachedSourceByLocalPath = cachedSources;
      _hasAttemptedValidation = false;
      _isInitialHandoverDocumentLoading = false;
      _errorMessage = null;
    });
  }
  ...
  void _clearHydratedFiles() {
    if (!mounted) return;
    setState(() {
      _existingHandoverDocumentFiles = [];
      _selectedImages = [];
      _selectedPdfFiles = [];
  ...
  String? _validationMessage() {
    ...
    if (_totalSelectedCount == 0) {
      return context.translate(i18.assetHandoverDocument.uploadRequired);
    }
  ...
  Map<PlatformFile, String?> _handlePdfFilesSelected(
    ...
    final total = _existingHandoverDocumentFiles.length +
        _selectedImages.length +
        files.length;
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      if ((file.path ?? '').isEmpty) {
        errors[file] = context.translate(
          i18.assetHandoverDocument.filePathUnavailable,
        );
  ...
  Future<void> _submitSelections() async {
    ...
    final inputs = <AssetHandoverDocumentInput>[];
    var index = 0;

    for (final existing in _existingHandoverDocumentFiles) {
    ...
    if (!mounted) return;
    context.read<CacheAssetHandoverDocumentBloc>().add(
          CacheAssetHandoverDocumentEvent.replaceAllForProject(
            activityFacilityId: _currentActivityFacilityId,
            files: inputs,
          ),
        );
  ...
  void _removeExistingHandoverDocumentFile(ExistingReport report) {
    setState(() {
      _existingHandoverDocumentFiles = _existingHandoverDocumentFiles
  ...
  Widget _buildExistingFilesSection() {
    return ExistingFilesOrLoader(
      existingReports: _existingHandoverDocumentFiles,
      workflowDocuments: const [],
      isLoading: _isInitialHandoverDocumentLoading,
      readOnly: _isViewOnly,
      onRemove: _removeExistingHandoverDocumentFile,
    );
  }
  ...
  Widget _buildUploadControls(ThemeData theme, dynamic textTheme) {
    ...
    return DigitCard(
      children: [
        ...
        if (!_isViewOnly)
          Text(
            context.translate(i18.assetHandoverDocument.uploadPrompt),
        ...
        if (!_isViewOnly) ...[
          ...
          if (_remainingPdfSlots > 0 || _selectedPdfFiles.isNotEmpty)
            FileUploadWidget(
              label: context.translate(i18.assetHandoverDocument.uploadPdf),
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<CacheAssetHandoverDocumentBloc,
        CacheAssetHandoverDocumentState>(
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      context.translate(
                        i18.assetHandoverDocument.title,
                      ),
