...
class _AssetSummaryPageState extends State<AssetSummaryPage> {
  ...
  /// Shows the send-back popup, allowing dynamic addition of reason rows.
//   void _showSendBackPopup2(BuildContext context) {
//     final theme = Theme.of(context);
//     final textTheme = theme.digitTextTheme(context);
//
//     _reasons
...
//             actionAlignment: MainAxisAlignment.center,
//             additionalWidgets: [
//               // For each reason entry, render a dropdown + textarea
//               ..._reasons.map((entry) {
//                 return Column(
//                   crossAxisAlignment: CrossAxisAlignment.stretch,
//                   children: [
//                     // inside your Popup.additionalWidgets:
//
// // assume you have a List<ReasonEntry> entries, and this is in a `...entries.map((entry) { ... }).toList()`
//                     Stack(
//                       clipBehavior: Clip.none,
//                       children: [
//                         // 1) The labeled dropdown itself:
//                         LabeledField(
//                           label: 'Reason',
//                           child: DigitDropdown(
//                             items: const [
...
//                         ),
//
//                         // 2) The little delete icon in top‑right:
//                         Positioned(
//                           top:
//                               -spacer3, // nudges icon slightly above the field’s top
//                           right:
//                               -spacer3, // nudges icon slightly beyond the right edge
//                           child: Material(
...
//                         Navigator.of(ctx).pop();
//
//                         // TODO: dispatch your send-back event with this payload
//                         // e.g. context.read<YourBloc>().add(SendBackEvent(payload));
//
//                         context.showSnackBar(
//                           SnackBar(
//                             content: Text(
//                               "Sent back with ${payload.length} reason(s).",
//                             ),
//                           ),
//                         );
//                       },
...
Widget editButton({
  ...
  return GestureDetector(
    ...
    child: Row(
      children: [
        Icon(Icons.edit,
            size: spacer5, color: Theme.of(context).colorTheme.alert.error),
        const SizedBox(width: spacer1),
        Text(
          context.translate(i18.common.coreCommonEdit),
          style: TextStyle(color: Theme.of(context).colorTheme.alert.error),
        ),
      ],
    ),
  );
